...
load(sprintf('%s/Intermediate/CovarianceMats/spatialAnalysisFullData.Rda',scratchDir))
...
# set.panel(1,3)
# decadeInd <- 3
# worldMap(lon, lat, monthlyOceanMask[,,1])
# worldMap(lon, lat, decadalLandCoverageArray[,,decadeInd])
# worldMap(lon, lat, possibleDataMask[,,1,decadeInd])


# # load an ind ensemble to check
# handle <- nc_open(filePaths[1])
# anom <- ncvar_get(handle, 'tempanomaly')
...
# set.panel(2,3)
# worldMap(lon, lat, monthlyOceanMask[,,1])
# worldMap(lon, lat, decadalLandCoverageArray[,,1])
# worldMap(lon, lat, possibleDataMask[,,1,1])

# divMap(lon,lat, anomSub[,,1])
# divMap(lon,lat, anomSub[,,13])
# divMap(lon,lat, anomSub[,,25])
...
singleEnsemble <- function(en){
	...
	for(d in 1:length(decInds)){
		...
		load(sprintf('%s/Intermediate/CovarianceMats/covMatFinal_%02d.Rda',scratchDir,d))
		...
		# generate all of the possibly needed random sampling uncertainty draws 
		# for the decade (Hard coded to deal with run through 2020 right now)
		if(d==14){
			rawSample <- rmvnorm(nSampling*12*11,rep(0,nrow(covMatFinal)),covMatFinal)
		} else{
			rawSample <- rmvnorm(nSampling*12*10,rep(0,nrow(covMatFinal)),covMatFinal)
		}
	...
	# Save step
	##############################
	memberInds <- 1:10 + (en-1)*10
	time <- timeMapSub[,3]

	# define dimensions
	londim      <- ncdim_def("lon","degrees_east",as.double(lon)) 
	latdim      <- ncdim_def("lat","degrees_north",as.double(lat)) 
	timeDim     <- ncdim_def("time",'Year',as.double(time))
	recordDim <- ncdim_def("record",'ensemble_member',as.double(memberInds),unlim=TRUE)
